...
namespace QuizCanners.Inspect
{
    public static partial class pegi
    {
        ...
        internal static ChangesToken InspectValueInList<T>(T el, List<T> list, int index, ref int inspected, CollectionInspectorMeta listMeta = null)
        {
        ...
        internal static ChangesToken InspectValueInHashSet<T>(T el, HashSet<T> list, int index, ref int inspected, CollectionInspectorMeta listMeta = null)
        {
            var tmp = el;
        ...
        public static ChangesToken AddDictionaryPairOptions<TValue>(Dictionary<string, TValue> dic, string defaultElementName) 
        {
            ...
            else
            {
                if (Icon.Add.Click("Add new Value"))
                {
                    ...
                    if (t.IsValueType || (typeof(Object).IsAssignableFrom(t) == false && t.GetConstructor(Type.EmptyTypes) != null))
        ...
        internal static ChangesToken Edit_Dictionary_Internal<TKey, TValue>(Dictionary<TKey, TValue> dic, System.Func<TValue, TValue> lambda, bool showKey = true, CollectionInspectorMeta listMeta = null)
        {
            ...
            if (listMeta != null && listMeta.IsAnyEntered)
            {
                ...
                if (listMeta.IsAnyEntered && (dic.Count > listMeta.InspectedElement))
                {
                    ...
                    var val = el.Value;
                    //var ch = GUI.changed;
                    var vaCh = pegi.ChangeTrackStart();

                    Nested_Inspect_Value_OrFallback(ref val);

                    if (vaCh && typeof(TValue).IsValueType) //new ChangesToken(!ch && GUI.changed))
                        dic[el.Key] = val;
                }
            }
            else
            ...
            if (inspected == -1)
            {
                ...
                foreach (var item in collectionInspector.InspectionIndexes(dic, listMeta, new KeyValuePairInspector<TKey, TValue>()))
                {
                    ...
                    if ((listMeta == null || listMeta[CollectionInspectParams.allowDeleting]) 
                        && Icon.Delete.ClickConfirm(confirmationTag: "DelDicEl"+collectionInspector.Index, toolTip: "Delete {0} (ID: {1})".F(item.Value, itemKey.ToString())))
                    {
                        dic.Remove(itemKey);
                ...
                if (count == 0)
                    "Empty Dictionary of {0} : {1}".F(typeof(TKey).ToPegiStringType(), typeof(TValue).ToPegiStringType()).PL(Styles.Text.Bald).NL();
        ...
        public class CollectionInspectorMeta : IPEGI, ICfg
        {
            ...
            internal void OnElementsDeleted() 
            {
                elementDatas.Clear();
            }
        ...
        internal class CollectionInspector : System.IDisposable
        {
                    ...
                    if (!_searching)
                    {
                        ...
                        if ((_sectionStartIndex > 0) || (_count > _lastElementToShow))
                        {
                            ...
                            if (_count > _lastElementToShow)
                            {
                                ...
                                " {0}{1}".F(SYMBOLS.X_SYMBOL, _count - _lastElementToShow).PL().Write();
                            }
                    ...
                    else
                    {
                        ...
                        if (_sectionStartIndex > 0 || gotToShow)
                        {
                            ...
                            if (gotToShow)
                            {
                                ...
                                if (!gotUnchecked)
                                    "{0}{1}".F(SYMBOLS.X_SYMBOL, filteredList.Count - _lastElementToShow).PL().Write();
            ...
            internal CollectionInspector Write_Search_DictionaryLabel<K, V>(TextLabel label, ref int inspected, Dictionary<K, V> dic, CollectionInspectorMeta meta = null)
            {
                ...
                else
                {
                    ...
                    if (Icon.ExitToList.ClickUnFocus("{0} {1}{2}".F(Msg.ReturnToCollection.GetText(), SYMBOLS.X_SYMBOL, dic.Count)))
                        inspected = -1;
                }
            ...
            internal CollectionInspector Write_Search_ListLabel<T>(TextLabel label, ref int inspected, ICollection<T> lst)
            {
                ...
                {
                    exitOptionHandled = true;
                    if (Icon.ExitToList.ClickUnFocus("{0} {1}{2}".F(Msg.ReturnToCollection.GetText(), SYMBOLS.X_SYMBOL, lst.Count)))
                        inspected = -1;
                }
            ...
            internal CollectionInspector Write_Search_ListLabel<T>(CollectionInspectorMeta ld, ICollection<T> lst)
            {
               ...
               // _toDispose.Add(Styles.Background.ListLabel.SetDisposible());
                ...
                {
                    exitOptionHandled = true;
                    if (Icon.ExitToList.ClickUnFocus("{0} {1} {2}{3}".F(Msg.ReturnToCollection.GetText(), currentListLabel, SYMBOLS.X_SYMBOL, lst.Count)))
            ...
            internal ChangesToken ExitOrDrawPEGI<T>(T[] array, ref int index, CollectionInspectorMeta ld = null)
            {
                ...
                if (index >= 0)
                {
                    if (!exitOptionHandled && (array == null || index >= array.Length || Icon.ExitToList.ClickUnFocus("Return to {0} array".F(GetCurrentListLabel<T>(ld))).NL()))
                        index = -1;
                    else
                    {
                        NL();
                        ...
                        if (Nested_Inspect_Object(ref obj))
                            array[index] = (T)obj;
                    }
                }
            ...
            internal ChangesToken ExitOrDrawPEGI<K, T>(Dictionary<K, T> dic, ref int index, CollectionInspectorMeta ld = null)
            {
                ...
                if (!exitOptionHandled && Icon.ExitToList.ClickUnFocus("{0}[{1}] of {2}".F(Msg.ReturnToCollection.GetText(), dic.Count, GetCurrentListLabel<T>(ld))).NL())
                    index = -1;
                else
                {
                    NL();
                    ...
                    if (Nested_Inspect_Value_OrFallback(ref value) && typeof(T).IsValueType)
                        dic[key] = value;

                 //   if (Nested_Inspect_Object(ref obj) && typeof(T).IsValueType)
                      // dic[key] = (T)obj;
                }
            ...
            internal ChangesToken ExitOrDrawPEGI<T>(List<T> list, ref int index, CollectionInspectorMeta ld = null)
            {
                ...
                if (!exitOptionHandled && Icon.ExitToList.ClickUnFocus("{0}[{1}] of {2}".F(Msg.ReturnToCollection.GetText(), list.Count, GetCurrentListLabel<T>(ld))).NL())
                    index = -1;
                else
                {
                    NL();
            ...
            internal ChangesToken Edit_Array_Order<T>(ref T[] array, CollectionInspectorMeta listMeta = null)
            {
                ...
                {
                    if ((listMeta == null || listMeta[CollectionInspectParams.showEditListButton]) && Icon.EditList.ClickUnFocus(Msg.MoveCollectionElements, 28))
            ...
            internal bool Edit_List_Order<T>(List<T> list, CollectionInspectorMeta listMeta = null)
            {
                ...
                if (list != reordering)
                {
                    if (!ReferenceEquals(sd.FilteredList, list) && (listMeta == null || listMeta[CollectionInspectParams.showEditListButton]) &&
                        Icon.EditList.ClickUnFocus(Msg.MoveCollectionElements, 28).IgnoreChanges(LatestInteractionEvent.Click))
                        reordering = list;
                }
...

                {
                    ...
                    foreach (var el in collectionInspector.InspectionIndexes(list, listMeta))
                    {
                        ...
                        if (listMeta == null || listMeta[CollectionInspectParams.allowDeleting])
                        {
                            ...
                            {
                                if (Icon.Delete.ClickUnFocus(Msg.RemoveFromCollection))
                                {
                                    list.RemoveAt(Index);
                                    Index--;
                                    _lastElementToShow--;
                ...
                if (selectedCount > 0 && "Unselect All".PL().Click().IgnoreChanges(LatestInteractionEvent.Click))
                    SetSelected(listMeta, list, false);

                if (selectedCount == 0 && list.Count > 0 && "Select All".PL().Click().IgnoreChanges(LatestInteractionEvent.Click))
                ...
                if (list != listCopyBuffer)
                {
                    ...
                    {
                        if (selectedCount > 0 &&
                            Icon.Delete.Click())//Confirm//("delLstPegi", list, "Delete {0} Selected".F(selectedCount)))
                        {
                            if (listMeta == null)
                            {
                                for (var i = list.Count - 1; i >= 0; i--)
                ...
                else if (typeof(Object).IsAssignableFrom(typeof(T)) || !listCopyBuffer.IsNullOrEmptyNonGeneric())
                {
