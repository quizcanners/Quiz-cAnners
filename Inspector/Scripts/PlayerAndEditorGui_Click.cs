...
namespace QuizCanners.Inspect
{
    public static partial class pegi
    {
        ...
        public static ChangesToken UnfocusOnChange(this ChangesToken token, LatestInteractionEvent evnt)
        {
            if (token)
                UnFocus();
            return token.IsChanged.FeedChanges_Internal(evnt);
        }
        ...
        public static ChangesToken ClickText(this TextLabel label, int fontSize)
        {
           ...
           // return textAndTip.ClickText(pegi.Styles.ScalableBlueText(fontSize));
        }
        ...
        internal static ChangesToken Click(this Sprite img, string toolTip, int size = defaultButtonSize)
        ...
        public static ChangesToken Click(Color col)
        {
            using (SetBgColorDisposable(Color.clear))
            {
                return icon.Empty.GUIColor(col).Click();
            }
        }
        ...
        {
#if UNITY_EDITOR
            if (sp && sp.Click(Msg.HighlightElement.GetText(), width).IgnoreChanges(LatestInteractionEvent.Click))
            {
